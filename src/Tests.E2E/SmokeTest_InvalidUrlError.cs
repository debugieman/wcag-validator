...
public class SmokeTest_InvalidUrlError : IAsyncLifetime
{
    ...
    [Fact]
    public async Task SmokeTest_ErrorMessage_ShouldAppear_WhenUrlUsesHttp()
    {
        ...
        await Expect(errorMessage).ToContainTextAsync("HTTPS");
    }

    [Fact]
    public async Task SmokeTest_ErrorMessage_ShouldAppear_WhenUrlHasNoProtocol()
    {
        await _page.GotoAsync("http://localhost:4200");
